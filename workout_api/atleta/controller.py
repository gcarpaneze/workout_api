...
@router.get(
    "/",
    summary="Buscar todas os atletas",
...
async def query(
    ...
    if nome != None:
        atletas = (
            (
                await db_session.execute(
                    select(AtletaModel).where(AtletaModel.nome == nome)
                )
            )
            .scalars()
            .all()
        ...
        atletas = (
            (
                await db_session.execute(
                    select(AtletaModel).where(AtletaModel.cpf == cpf)
                )
            )
            .scalars()
            .first()
...
@router.get(
    "/{id}",
    summary="Buscar um atleta pelo ID",
    status_code=status.HTTP_200_OK,
    response_model=AtletaOut,
)
async def query(id: UUID4, db_session: DatabaseDependency) -> AtletaOut:
    ...
    if not atleta:
        raise HTTPException(
            ...
            detail=f"Atleta com o {id} não encontrado",
        )
